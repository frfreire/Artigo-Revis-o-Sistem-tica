...
\begin{table}[!t]
\caption{Comparativo de Efetividade Média por Técnica (Meta-Análise Atualizada)}
...
\toprule
\textbf{Técnica Predominante} & \textbf{F1-Score} & \textbf{FPR} & \textbf{Latência} & \textbf{Métrica Alternativa} \\
\midrule
ML Clássico (RF/SVM) & 94.2\% & 2.8\% & Baixa ($\mu$s) & — \\
Deep Learning (CNN+LSTM) & \textbf{97.8\%} & 1.1\% & Média (ms) & — \\
Graph Neural Nets (GNN) & 99.1\% & <0.5\% & Alta (ms) & Robustez anti-ofuscação \\
GenAI/LLMs (Agentes) & N/A\textsuperscript{*} & N/A\textsuperscript{*} & Muito Alta (s) & MTTR: -50\% ($\pm$5\%); Alucinação: <8\% \\
\bottomrule
\multicolumn{5}{p{0.95\columnwidth}}{\scriptsize{\textsuperscript{*}LLMs em contextos de SecOps são avaliados por redução de tempo médio de resposta (MTTR) e qualidade de triagem, não por métricas de classificação binária tradicional \cite{capgemini2024, habibzadeh2025}.}}
\end{tabular}%
}
\end{table}
